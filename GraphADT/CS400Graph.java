...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
        	...
        	// adds every element in the original to the copy
        	for(int i = 0; i < source.dataSequence.size(); i++) {
        		copy.add(source.dataSequence.get(i));
        	}
        	
        	//assigns start and end as the first and last elements in copy
        	this.start = new Vertex(copy.get(0));
        	this.end = new Vertex(copy.get(copy.size()-1));
        	
        	
        	this.dataSequence = copy;
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
    	...
    	//throws a NSEE if start or end are null, or if start is also the end,
    	// or if the end or start exist
    	if (start == null || end == null || isEmpty() || start == end || !containsVertex(start) || !containsVertex(end)) {
    		throw new NoSuchElementException("No path exists");
    	}
    	
        Path cPath = new Path(vertices.get(start));
        Path sPath = null; //to store the shortest path found
        ...
        pq.add(cPath);
        visited.put(start, cPath.start);
        
		while (!pq.isEmpty()) {

			//goes through every edge in the node 
			for (int i = 0; i <= cPath.end.edgesLeaving.size() - 1; i++){
					Path copy = new Path(cPath);
					copy.extend(cPath.end.edgesLeaving.get(i));
					
					//checks if edge was visited, if not add it to pq
					if (!visited.contains(copy.end) || copy.end.data.equals(end)){
						pq.add(copy);
						visited.put(copy.end.data, copy.end );
					}	
			}
			//checks if the path does reach end
			if (cPath.end.data.equals(end)) {
				//if this is the first path found then it saves it,
				//otherwise compares the od path and the new path and saves the shortest
				if (sPath == null || sPath.compareTo(cPath) > 0){
					sPath = cPath;
				}
			}
        ...
        //throws NSEE if path was not found
        if (sPath == null || !sPath.start.data.equals(start) || !sPath.end.data.equals(end) ) {
        	throw new NoSuchElementException("No path found");
        }
        
        return sPath; // returns shortest path found
    }
